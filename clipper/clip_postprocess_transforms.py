...
def interpolate_to_target(
    ...
    """Use RIFE to interpolate frames up to *target_count*.

    Iteratively doubles the frame count by inserting RIFE-interpolated
    midpoints between each consecutive pair until we have >= target_count,
    then evenly samples down to exactly target_count.  Falls back to
    linear blending if RIFE is unavailable.
    """
    if len(frames) >= target_count:
        return frames

    setup = _rife_setup()
    use_rife = setup is not None

    work = list(frames)
    tmpdir = None
    if use_rife:
        tmpdir = tempfile.mkdtemp(prefix="rife_interp_")

    try:
        while len(work) < target_count:
            doubled: list[np.ndarray] = []
            for i in range(len(work)):
                doubled.append(work[i])
                next_idx = (i + 1) % len(work)
                if use_rife:
                    rife_exe, model_dir = setup
                    mid = _rife_interpolate_frame(
                        rife_exe, model_dir,
                        work[i], work[next_idx],
                        0.5, tmpdir, tag=f"d{len(doubled)}",
                    )
                    if mid is not None:
                        doubled.append(mid)
                        continue
                doubled.append(blend_pair(work[i], work[next_idx], 0.5))
            work = doubled
    finally:
    ...
    indices = [int(round(i * (len(work) - 1) / (target_count - 1))) for i in range(target_count)]
    return [work[i] for i in indices]


def upscale_frames(frames: list[np.ndarray], scale: float) -> list[np.ndarray]:
    """Scale frames up by *scale* (must be > 1.0). Uses INTER_LANCZOS4."""
    if scale <= 1.001:
        return frames
    ...
    return [cv2.resize(frame, (new_w, new_h), interpolation=cv2.INTER_LANCZOS4) for frame in frames]
