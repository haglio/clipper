...
from clipper.window_runtime import cleanup_window, ensure_window, window_closed
...
def test_ensure_window_creates_window_sets_icon_and_mouse_callback():
    ...
    with patch("clipper.window_runtime.set_cv2_window_icon") as set_icon:
...
def test_cleanup_window_terminates_export_releases_capture_and_drains_waitkeys():
    ...
    with patch("clipper.window_runtime.terminate_export_subprocesses") as terminate_export:
...
def test_cleanup_window_stops_waitkey_drain_after_cv2_error():
    ...
    with patch("clipper.window_runtime.terminate_export_subprocesses"):
        cleanup_window("Clipper", state, cv2_module=fake_cv2, sleep=lambda _seconds: None)
