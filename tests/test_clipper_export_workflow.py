...
from clipper.export import start_export_job, terminate_export_subprocesses
...
def test_start_export_job_returns_early_when_job_already_active():
    ...
    with patch("clipper.export.start_daemon_thread") as start_thread:
...
def test_start_export_job_completes_successful_pipeline(tmp_path: Path):
    ...
    with patch("clipper.export.RAW_CLIPS_DIR", tmp_path), \
         patch("clipper.export.CLIPS_DIR", tmp_path), \
         patch("clipper.export.AUDIO_DIR", tmp_path), \
         patch("clipper.export.export_raw_clip", return_value=(True, "raw")) as export_raw, \
         patch("clipper.export.run_clip_postprocess", return_value=(True, "clip")) as postprocess, \
         patch("clipper.export.export_full_audio_mp3", return_value=(True, "audio")) as export_audio, \
         patch("clipper.export.start_daemon_thread", side_effect=lambda **kwargs: _run_export_worker_immediately(**kwargs) or fake_thread):
        start_export_job(state)
...
def test_start_export_job_marks_raw_clip_failure(tmp_path: Path):
    ...
    with patch("clipper.export.RAW_CLIPS_DIR", tmp_path), \
         patch("clipper.export.CLIPS_DIR", tmp_path), \
         patch("clipper.export.AUDIO_DIR", tmp_path), \
         patch("clipper.export.export_raw_clip", return_value=(False, "raw failed")) as export_raw, \
         patch("clipper.export.run_clip_postprocess") as postprocess, \
         patch("clipper.export.export_full_audio_mp3") as export_audio, \
         patch("clipper.export.start_daemon_thread", side_effect=_run_export_worker_immediately):
        start_export_job(state)
...
def test_start_export_job_marks_postprocess_failure(tmp_path: Path):
    ...
    with patch("clipper.export.RAW_CLIPS_DIR", tmp_path), \
         ...
         patch("clipper.export.AUDIO_DIR", tmp_path), \
         patch("clipper.export.export_raw_clip", return_value=(True, "raw")) as export_raw, \
         patch("clipper.export.run_clip_postprocess", return_value=(False, "post failed")) as postprocess, \
         patch("clipper.export.export_full_audio_mp3") as export_audio, \
         patch("clipper.export.start_daemon_thread", side_effect=_run_export_worker_immediately):
        start_export_job(state)
...
def test_start_export_job_marks_audio_failure(tmp_path: Path):
    ...
    with patch("clipper.export.RAW_CLIPS_DIR", tmp_path), \
         ...
         patch("clipper.export.export_raw_clip", return_value=(True, "raw")) as export_raw, \
         patch("clipper.export.run_clip_postprocess", return_value=(True, "clip")) as postprocess, \
         patch("clipper.export.export_full_audio_mp3", return_value=(False, "audio failed")) as export_audio, \
         patch("clipper.export.start_daemon_thread", side_effect=_run_export_worker_immediately):
        start_export_job(state)
